...
// --- Login View (保留您的微調) ---
const LoginView = ({ onGoogleLogin, onGuestLogin, theme }) => (  <div className={`flex flex-col items-center justify-center h-screen ${theme.light} p-6`}>
  ...
  const unsubTx = onSnapshot(
    ...
    (snapshot) => {
      const docs = snapshot.docs.map(doc => {
        const raw = doc.data();
        const createdAt = raw.createdAt?.toDate?.() || raw.date?.toDate?.() || (raw.date instanceof Date ? raw.date : null);
        return {
          id: doc.id,
          ...raw,
          date: createdAt || new Date(raw.date || Date.now())
        };
      });
...
// Actions
  ...
  const saveTx = async (data) => {
    try {
      ...
      const txnDate = dataToSave.date instanceof Date ? dataToSave.date : new Date(dataToSave.date);
      const payload = {
        ...dataToSave,
        amount: Number(dataToSave.amount),
        accountId: dataToSave.accountId || accounts[0]?.id,
        createdAt: txnDate,
        date: txnDate,
        excludeFromMonthly: dataToSave.type === 'expense' ? Boolean(dataToSave.excludeFromMonthly) : false
      };
      // 🔥 [修正路徑]
  ...
  return (
      <div className="p-5 pb-24 space-y-6">
          <div className="flex justify-between items-center">
              ...
              <button 
                onClick={() => { setIsSelectionMode(!isSelectionMode); setSelectedIds(new Set()); }}                className={`text-xs px-3 py-1.5 rounded-lg border font-bold ${isSelectionMode ? `bg-gray-700 text-white border-gray-700` : `border-gray-200 text-gray-500`}`}
              >
